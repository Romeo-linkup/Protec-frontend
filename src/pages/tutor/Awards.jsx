...
const COMMENTS = {
  ...
  pass: ["Good effort. Every mark counts — keep going!","You're making progress. Stay consistent.","Passing is just the beginning — aim higher next term.","Keep showing up and putting in the work.","Small improvements add up to big wins."],
  below: ["Don't give up — every challenge is a chance to grow.","We believe in you. Let's work together to turn this around.","Struggles now make the success sweeter later.","You are more capable than these marks show.","This is not your ceiling. Keep fighting."],
};
      ...
      {data.slice(0, 10).map((x, i) => {
        return (
          <div key={x.learner_id||x.id} style={{
            ...
            padding:'10px 12px', display:'flex', alignItems:'flex-start', gap:'10px',
          }}>
            {i < 3 ? (
...
export default function TutorAwards() {
  ...
  const t1Map={}; t1.forEach(r=>{t1Map[r.learner_id]=overall(r);});
  const src = grade ? t2.filter(r=>r.grade===grade) : t2;
  const withAvg = src.map(r=>({...r,avg:overall(r)})).filter(x=>x.avg!=null).sort((a,b)=>b.avg-a.avg);
  const improved = withAvg.map(r=>({...r,diff:t1Map[r.learner_id]!=null?r.avg-t1Map[r.learner_id]:null})).filter(x=>x.diff!=null).sort((a,b)=>b.diff-a.diff);
