  ...
  return (
    <div className="view">
      <div className="topbar">
        <div>
          <div className="topbar-title">All results</div>
          <div className="topbar-sub">{filtered.length} learner{filtered.length!==1?'s':''}</div>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Math (Sch)</th><th>Math (Pro)</th><th>Math avg</th>
                <th>Sci (Sch)</th><th>Sci (Pro)</th><th>Sci avg</th>
                <th>Eng (Sch)</th><th>Eng (Pro)</th><th>Eng avg</th>
                <th>Overall</th><th>Attendance</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length===0 ? (
                <tr><td colSpan="13" className="no-data">No results found.</td></tr>
              ) : filtered.map(r => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td>{markEl(r.math_school)}</td><td>{markEl(r.math_protec)}</td><td>{markEl(ma)}</td>
                    <td>{markEl(r.sci_school)}</td><td>{markEl(r.sci_protec)}</td><td>{markEl(sa)}</td>
                    <td>{markEl(r.eng_school)}</td><td>{markEl(r.eng_protec)}</td><td>{markEl(ea)}</td>
                    <td>{markEl(oa)}</td>
                    <td>{attEl(att)}</td>
                  </tr>
                );
